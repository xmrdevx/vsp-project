...
import { AuthenticatedUser, AuthenticatedStatus, RefreshTokenRequest } from '@vsp/core';
...
export class JwtTokenInterceptor implements HttpInterceptor, OnDestroy {
  ...
  private _authenticatedUserSubscription: Subscription = this._authenticationStore
    .authenticatedUser$
  ...
  private _handleAccessTokenRefreshing(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    if (!this._isRefreshing) {
      ...
      this._authenticationStore.refreshAccessToken({
      ...
      return combineLatest([
        ...
        this._authenticationStore.refreshAccessTokenResponseMessage$
      ]).pipe(
        skip(1),
  ...
  private _handleRefreshAccessTokenFailure(): void {
    this._isRefreshing = false;
    this._authenticationStore.setAuthenticatedUser(null);
  }
