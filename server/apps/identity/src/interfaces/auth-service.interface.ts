...
export interface IAuthService {
   ...
   * @param {UserDetails} user The users details of the user being signed in.
   * @returns {Promise<AuthenticatedUser>} The authenticated user with autenticated status and pair of tokens.
   * @async
   * @abstract
   */
   ...
   * @param {Credentials} credentials The credentials of the user being validated for sign in.
   * @returns {Promise<UserDetails | null>} The user details of validation passes else returns null (validation doesn't pass).
   * @async
   * @abstract
   */
   ...
   * @param {TokenPair} tokens The current expired access token and a refresh token
   * @returns {Promise<TokenPair>} A new token pair with a valid access token and refresh token.
